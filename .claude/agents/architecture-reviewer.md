...
1. **Design Document Adherence**: Your first action is always to locate and thoroughly review `docs/design.md` and any related design documentation in the `docs/` directory and per-package `docs/design.md` files. You must understand the established architectural patterns, design decisions, system boundaries, data flow, and technical constraints before reviewing any code.
...
- Review per-package design docs in `src/<module>/<package>/docs/design.md`
...
- Does it follow established architectural patterns (Builder, Disposable, Result, Extension, Strategy)?
...
- Example: `src/Observability/Runtime/Scopes/InvokeAgentScope.cs`
...
- Reference specific locations: `[MethodName](path/to/file.cs#L42)`
