...
public class PowerPlatformApiDiscoveryTests
{
    ...
    public void GetTokenAudience_Mapping_IsCorrect()
    {
        ...
        {
            ["local"] = "https://api.powerplatform.localhost",
            ["dev"] = "https://api.dev.powerplatform.com",
        ...
        {
            var disc = new PowerPlatformApiDiscovery("local");
        ...
        {
            var discLocal = new PowerPlatformApiDiscovery("local");
            var ex1 = Assert.Throws<ArgumentException>(() => discLocal.GetTenantEndpoint("a"));
            Assert.Contains("must be at least 2 characters in length", ex1.Message);

            var ex2 = Assert.Throws<ArgumentException>(() => discLocal.GetTenantEndpoint("a-"));
            Assert.Contains("must be at least 2 characters in length", ex2.Message);
